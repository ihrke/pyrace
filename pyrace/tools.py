...
list_is_eq=lambda l1,l2: len(set(l1).intersection(l2)) > 0
...
def pnormP(x, mean=0, sd=1):
    ...
    """
    return np.where(np.abs(x)<7, stats.norm.cdf(x, loc=mean,scale=sd), np.where(x<0,0,1))
def dnormP(x, mean=0, sd=1):
    ...
    """
    return np.where(np.abs(x)<7,stats.norm.pdf(x, loc=mean, scale=sd),0)
